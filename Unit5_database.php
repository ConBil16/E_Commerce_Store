...
function getConnection(){
    require 'Unit5_database_credentials.php';
...
<?php
    ...
    function orderExists($conn, $productId){
    ...
    function updateProduct ($conn, $productName, $imageName, $quantity, $price, $inactive, $id){
        $query = "UPDATE product SET product_name = ?, image_name =?, price = ?, in_stock = ?, inactive = ? WHERE id = ?";
        $stmt = $conn->prepare( $query );
        $stmt->bind_param("ssdiii", $productName, $imageName, $price, $quantity, $inactive, $id);
        $stmt->execute();
        $stmt->close();
    }
